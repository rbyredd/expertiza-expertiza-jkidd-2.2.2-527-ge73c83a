module GradesHelper
  ...
  def find_question_types(question, ques_type, q_number, is_view, file_url, score, score_range)
    ...
    case ques_type.q_type
      ...
      when "TextField"
        ...
        render :partial => "response/textfield", :locals => {:ques_num => q_number, :field_size => size, :ques_text => question.txt, :separator => separator, :isFirst => is_first, :isLast => is_last, :view => find2(score)}
      when "TextArea"
        #Parameters
        ...
        render :partial => "response/textarea", :locals => {:ques_num => q_number, :area_size => size, :ques_text => question.txt, :table_title => table_hash["table_title"], :table_headers => table_hash["table_headers"], :start_col => table_hash["start_col"], :start_table => table_hash["start_table"], :end_col => table_hash["end_col"], :end_table => table_hash["end_table"], :view => view_scores(score)}
      when "UploadFile"
        #Parameters
        #section

        render :partial => "response/fileUpload", :locals => {:ques_num => q_number, :ques_text => question.txt, :view => view_fileupload(file_url)}
      when "DropDown"
        #Parameters
  ...
  def view_checkbox(question,score)
    view_output = nil
    if is_view
