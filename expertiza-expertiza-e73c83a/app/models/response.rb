class Response < ActiveRecord::Base
  ...
  def display_as_html(prefix = nil, count = nil, file_url = nil)
    ...
    code = identifier + display_header(prefix, count)
    ...
    code += display_scores()
    code += display_additional_comment()

    return code
  end

  def isFeedback?()
        return self.map.type.to_s == 'FeedbackResponseMap'
  end

  def display_header(prefix = nil, count = nil)
    identifier = ""
    code = ""
    if prefix
  ...
  def display_additional_comment()
    code = ""
